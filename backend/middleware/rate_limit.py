...
def get_redis_connection():
    ...
    if not redis_enabled:
        logger.info("Redis disabled via REDIS_ENABLED=false, using in-memory rate limiting")
        return None
...
limiter = Limiter(
    ...
    default_limits=[f"{default_ip_limit}/hour"]  # Configurable requests per hour per IP
)
...
class SimpleRateLimiter:
    def __init__(self):
        ...
        self.window = int(os.getenv("RATE_LIMIT_WINDOW", "3600"))  # Configurable window
...
class TokenTracker:
    ...
    def __init__(self):
        ...
        self.ip_token_limit = int(os.getenv("RATE_LIMIT_IP_TOKENS", "10000"))  # per hour
        self.global_token_limit = int(os.getenv("RATE_LIMIT_GLOBAL_TOKENS", "100000"))  # per hour
        self.window = int(os.getenv("RATE_LIMIT_WINDOW", "3600"))  # Configurable window
    
    async def check_token_limits(self, ip: str, estimated_tokens: int) -> Tuple[bool, str]:
        ...
        if ip_tokens + estimated_tokens > self.ip_token_limit:
            return False, f"IP token limit exceeded. Used: {ip_tokens}/{self.ip_token_limit}"
        ...
        if global_tokens + estimated_tokens > self.global_token_limit:
            return False, f"Global token limit exceeded. Used: {global_tokens}/{self.global_token_limit}"
    ...
    async def consume_tokens(self, ip: str, tokens: int):
        ...
        await self.redis.expire(ip_key, self.window)  # Expire after configured window
...
async def token_limit_middleware(request: Request, call_next):
    ...
    if not simple_rate_limiter.check_rate_limit(client_ip):
        ...
        return JSONResponse(
            ...
            content={
                ...
                "message": f"Too many requests. Limit: {simple_rate_limiter.limit} requests per {simple_rate_limiter.window} seconds",
            ...
            if body:
                try:
                    ...
                    if "message" in data:
                        ...
                        if "current_file_content" in data and data["current_file_content"]:
                            estimated_tokens += estimate_tokens(data["current_file_content"])
    ...
    if estimated_tokens > 0:
        allowed, error_msg = await token_tracker.check_token_limits(client_ip, estimated_tokens)
        if not allowed:
            logger.warning(
    ...
    if estimated_tokens > 0:
        response.headers["X-RateLimit-Estimated-Tokens"] = str(estimated_tokens)

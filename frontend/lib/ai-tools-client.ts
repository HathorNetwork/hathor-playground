...
export class AIToolsClient {
  ...
  /**
   * Get project structure as a hierarchical tree
   */
  static async getProjectStructure(filterByType?: 'blueprints' | 'tests' | 'dapp' | 'components' | 'pages' | 'configs'): Promise<ToolResult> {
    try {
      const files = useIDEStore.getState().files;
      ...
      if (filterByType) {
        switch (filterByType) {
          case 'blueprints':
            filteredFiles = files.filter(f => f.path.startsWith('/blueprints/') || f.path.startsWith('/contracts/'));
            break;
          case 'tests':
          ...
          case 'components':
            ...
            break;
          case 'pages':
            filteredFiles = files.filter(f => f.path.includes('/app/') && (f.path.endsWith('page.tsx') || f.path.endsWith('layout.tsx')));
            break;
          case 'configs':
      ...
      const components = files.filter(f => f.path.includes('/components/') && f.path.endsWith('.tsx'));
      const pages = files.filter(f => f.path.includes('/app/') && (f.path.endsWith('page.tsx') || f.path.endsWith('layout.tsx')));
  ...
  /**
   * Find files by name pattern (fuzzy matching)
   */
  static async findFile(pattern: string, searchPath?: string): Promise<ToolResult> {
    try {
      const files = useIDEStore.getState().files;
      
      // Normalize pattern (lowercase, remove extensions for fuzzy matching)
      ...
      const matches = searchFiles.map(file => {
      ...
      const message = [
        ...
        '',
        ...matches.map(m => `  ${m.file.path} (${m.file.type}, ${m.file.content.length} bytes, match score: ${m.score})`),
      ].join('\n');
  ...
  /**
   * Get file dependencies (imports/exports)
   */
  static async getFileDependencies(filePath: string): Promise<ToolResult> {
    try {
      const files = useIDEStore.getState().files;
        ...
        if (importPath.startsWith('./') || importPath.startsWith('../')) {
          ...
          const resolved = importPath
            ...
            .replace(/^\.\.\//g, (match, offset) => {
              const upDir = dir.substring(0, dir.lastIndexOf('/'));
              return upDir + '/';
            });
      ...
      for (const file of files) {
        ...
        const importPatterns = [
          new RegExp(`from\\s+['"]\\.\\.?/[^'"]*${fileName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`, 'i'),
   ...
   */
  static async analyzeComponent(filePath: string): Promise<ToolResult> {
    try {
      const files = useIDEStore.getState().files;
      ...
      const componentNameMatch = content.match(/(?:export\s+(?:default\s+)?(?:function|const)\s+(\w+)|export\s+default\s+(\w+)|const\s+(\w+)\s*=\s*(?:\([^)]*\)\s*=>|function))/);
      const componentName = componentNameMatch?.[1] || componentNameMatch?.[2] || componentNameMatch?.[3] || 'Unknown';
      ...
      const componentImportPattern = new RegExp(
        `import\\s+.*\\b${componentName}\\b.*from\\s+['"][^'"]*${filePath.split('/').pop()?.replace(/\.(tsx?|jsx?)$/, '')}['"]`,
        'i'
      );
      ...
      const message = [
        ...
        hasUseClient === false && (usesHooks || hasEventHandlers) ? `⚠️ WARNING: Component needs "use client" directive!` : '',
  ...
  /**
   * Integrate a component into the application
   * Automatically adds component to appropriate page/route and updates imports
   */
  static async integrateComponent(componentPath: string, targetPage?: string): Promise<ToolResult> {
    try {
      const files = useIDEStore.getState().files;
      const { updateFile, addFile } = useIDEStore.getState();
      ...
      const componentNameMatch = componentFile.content.match(/(?:export\s+(?:default\s+)?(?:function|const)\s+(\w+)|export\s+default\s+(\w+)|const\s+(\w+)\s*=\s*(?:\([^)]*\)\s*=>|function))/);
      const componentName = componentNameMatch?.[1] || componentNameMatch?.[2] || componentNameMatch?.[3] || componentPath.split('/').pop()?.replace(/\.(tsx?|jsx?)$/, '') || 'Component';
      
      // Determine target page if not specified
      let targetPagePath = targetPage;
      if (!targetPagePath) {
        // Default to app/page.tsx for Hathor dApps
        const dappPages = files.filter(f => 
        ...
        if (dappPages.length > 0) {
          // Find the main page (usually /dapp/hathor-dapp/app/page.tsx)
          targetPagePath = dappPages.find(p => p.path.includes('hathor-dapp/app/page.tsx'))?.path || dappPages[0].path;
        } else {
          return {
            success: false,
      ...
      // Try @/ alias if it's a dApp file
      if (componentPath.startsWith('/dapp/')) {
        const aliasPath = componentPath.replace('/dapp/', '@/').replace(/\.(tsx?|jsx?)$/, '');
        importPath = aliasPath;
      }
      
      // Check if import already exists
      const importPattern = new RegExp(`import\\s+.*\\b${componentName}\\b.*from\\s+['"]${importPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}['"]`, 'i');
      const hasImport = importPattern.test(targetPageFile.content);
      
      // Check if component is already used in the page
      const isUsed = targetPageFile.content.includes(`<${componentName}`) || targetPageFile.content.includes(`<${componentName} `);
      ...
      if (!isUsed) {
        // Try to find a good place to add the component (inside return statement)
        ...
        if (returnMatch) {
          ...
          // Fallback: add at the end of the component body
  ...
  static async syncDApp(
    ...
    try {
      ...
      if (sandboxHasChanges && (direction === 'sandbox-to-ide' || direction === 'bidirectional')) {
        ...
        // If the commit doesn't exist in sandbox, it means the sandbox was reset/recreated
        // In this case, we should compare against current sandbox HEAD instead
        // The getSandboxChangedFiles function already handles this by returning all files as "added"
        // So we can proceed normally - all sandbox files will be synced to IDE
        const sandboxResponse = await fetch(`/api/beam/sandbox/${activeProjectId}/files`);
        ...
        for (const changedFile of sandboxChangedFiles) {
          ...
          if (changedFile.status === 'deleted') {
            ...
            // Note: We can't use lastSyncedHash (sandbox commit) to query IDE repo
            // Instead, check if file has uncommitted changes or was recently modified
            if (existingFile && ideHasChanges) {
              // Check if this specific file has uncommitted changes
              const ideFileChanged = existingFile.content !== sandboxContent;
      ...
      // Always push to sandbox if direction is ide-to-sandbox or bidirectional
      // Don't check ideHasChanges - we want to sync all files regardless
      if (direction === 'ide-to-sandbox' || direction === 'bidirectional') {
        // Push changes to sandbox
        addConsoleMessage?.('info', '📤 Pushing changes to sandbox...');
        console.log(`[SYNC] Pushing to sandbox: direction=${direction}, dappFiles.length=${dappFiles.length}`);

        // Commit IDE changes first (if any)
        if (ideHasChanges) {
          await GitService.commit(activeProjectId, 'IDE changes before sync', dappFiles);
        }

        // Build files to upload - always upload ALL files when pushing to sandbox
        // This ensures everything is synced correctly regardless of Git state
        const ideFilesToUpload: Record<string, string> = {};
        
        console.log(`[SYNC] Building upload list from ${dappFiles.length} dApp files...`);
        
        // Always upload all IDE files when pushing to sandbox
        ...
        console.log(`[SYNC] Prepared ${Object.keys(ideFilesToUpload).length} files to upload`);
        console.log(`[SYNC] Sample files:`, Object.keys(ideFilesToUpload).slice(0, 5));

        if (Object.keys(ideFilesToUpload).length > 0) {
          try {
            console.log(`[SYNC] Uploading ${Object.keys(ideFilesToUpload).length} files to sandbox...`);
            await beamClient.uploadFiles(activeProjectId, ideFilesToUpload, false);
            console.log(`[SYNC] Files uploaded successfully, committing sandbox state...`);
            ...
            addConsoleMessage?.('success', `✅ Successfully uploaded ${Object.keys(ideFilesToUpload).length} files to sandbox`);
          } catch (uploadError: any) {
            console.error(`[SYNC] Failed to upload files:`, uploadError);
            addConsoleMessage?.('error', `❌ Failed to upload files: ${uploadError.message || String(uploadError)}`);
            throw uploadError; // Re-throw to be caught by outer try-catch
          }
        } else {
          console.warn(`[SYNC] No files to upload! dappFiles.length = ${dappFiles.length}`);
          addConsoleMessage?.('warning', '⚠️ No files found to upload to sandbox');
        }
      }
